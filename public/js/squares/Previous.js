...
export class Previous {
    ...
    static blockType = 'previous';
    static fallbackColor = 0x8fbcbb; 
    ...
    static createInstance(position) {
        ...
        group.add(square);
        group.userData = { blockType: this.blockType, previousAmount: this.defaultPreviousAmount };
    ...
    static populateContextMenu(menuElement, blockGroup) {
        ...
        const currentAmount = blockGroup.userData.previousAmount ?? this.defaultPreviousAmount;
        ...
        label.htmlFor = 'previous-amount-input'; label.textContent = 'Amount: '; label.style.marginRight = '5px';
        const input = document.createElement('input');
        input.type = 'number'; input.id = 'previous-amount-input'; input.className = 'menu-input';
        ...
        input.addEventListener('change', (event) => {
            ...
            blockGroup.userData.previousAmount = value;
            title.textContent = `${this.label} (-${value})`;
            console.log(`Previous amount set to: ${value}`);
        });
    ...
    static onCursorStep(blockInstance, currentGridPos, previousGridPos, blockGridMap, blockRegistry, cursor) {
        ...
        const previousAmount = blockInstance.userData?.previousAmount ?? this.defaultPreviousAmount;
        const oldPointer = cursor.indexPointer;
        cursor.decrementIndexPointer(previousAmount);
        console.log(`${this.label} at ${currentGridKey}: Decrementing index pointer by ${previousAmount}. Old: ${oldPointer}, New: ${cursor.indexPointer}`);
