...
export class RandomDirection {
    ...
    static onCursorStep(blockInstance, currentGridPos, previousGridPos, blockGridMap, blockRegistry) {
        ...
        if (!DirClass) {
            console.error("RandomDirection Error: Could not find 'director' block class in registry. Stopping.");
        ...
        if (!possibleDirs || !Array.isArray(possibleDirs) || possibleDirs.length === 0) {
            console.error("RandomDirection Error: Invalid or empty 'dirs' array found on directorSquare class. Stopping.");
            return { action: 'fizzleInPlace', nextGridPos: null };
        }

        const randomIndex = Math.floor(Math.random() * possibleDirs.length);
        const randomDirInfo = possibleDirs[randomIndex];
        if (!randomDirInfo || !randomDirInfo.vec || typeof randomDirInfo.rotation !== 'number' || typeof randomDirInfo.name !== 'string') {
             console.error("RandomDirection Error: Invalid direction info found in directorSquare.dirs array. Stopping.");
             return { action: 'fizzleInPlace', nextGridPos: null };
        }
        const directionVec = randomDirInfo.vec;
        console.log(`RandomDirection at ${currentGridKey} chose: ${randomDirInfo.name}`);

        const targetRotation = randomDirInfo.rotation;
        ...
        if (nextBlockKey === null) {
             console.warn(`RandomDirection calculated invalid next pos: ${JSON.stringify(nextGridPos)}`);
        ...
        if (nextBlockExists) {
            ...
            return { action: 'move', nextGridPos: nextGridPos };
             ...
             console.log(`RandomDirection: No block at random next key [${nextBlockKey}] (chosen dir: ${randomDirInfo.name}). Fizzling.`);
             return { action: 'moveAndFizzle', nextGridPos: nextGridPos };
        }
    }
}