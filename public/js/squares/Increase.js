...
export class Increase {
    ...
    static blockType = 'increase'; 
    static fallbackColor = 0xa3be8c; 
    static label = 'Increase'; 
    static texturePath = '/textures/increase.png';
    static defaultIncreaseAmount = 1; 
    ...
    static createInstance(position) {
        ...
        group.userData = {
            ...
            increaseAmount: this.defaultIncreaseAmount
        };
    ...
    static populateContextMenu(menuElement, blockGroup) {
        ...
        const currentAmount = blockGroup.userData.increaseAmount ?? this.defaultIncreaseAmount;
        ...
        label.htmlFor = 'increase-amount-input';
        label.textContent = 'Amount: '; label.style.marginRight = '5px';
        const input = document.createElement('input');
        input.type = 'number';

        input.id = 'increase-amount-input';
        ...
        input.addEventListener('change', (event) => {
            ...
            blockGroup.userData.increaseAmount = value;
            title.textContent = `${this.label} (+${value})`;
            console.log(`Increase amount set to: ${value}`); 
        });
    ...
    /**
     * Increases the cursor's current index value with wrap-around (0-255)
     * and tells the cursor to continue in the same direction it entered from.
    ...
    static onCursorStep(blockInstance, currentGridPos, previousGridPos, blockGridMap, blockRegistry, cursor) {
        ...
        const increaseAmount = blockInstance.userData?.increaseAmount ?? this.defaultIncreaseAmount;

        const currentValue = cursor.getCurrentIndexValue(); 

        const newValue = (currentValue + increaseAmount) % 256;

        cursor.setCurrentIndexValue(newValue); 

        console.log(`${this.label} at ${currentGridKey}: Increasing index by ${increaseAmount}. Old: ${currentValue}, New: ${newValue}`);
